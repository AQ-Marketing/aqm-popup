...
- **Removed the stray white border around the close button.** The popup focuses the close button when it opens (for keyboard/screen-reader users), which was drawing a white focus outline that appeared to flicker in and out. The focus outline is gone, so the X button stays clean.
...
- **Note:** the jump *to* 1.2.2 is still handled by the old updater, so that one update may double one last time. From 1.2.2 onward, updates apply in a single click.
